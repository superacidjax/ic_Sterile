module FabricationMethods
  ...
  def create_from_table(model_name, table, extra = {})
    ...
    @they = hashes.map do |hash|
      hash = parameterize_hash(hash.merge(extra), model_name.classify.constantize)
  ...
  def parameterize_hash(hash, klass)
    hash.inject({}) do |h,(k,v)|
      v = v.split(/\s*,\s*/) if klass.serialized_attributes[k] == Array
      h.update(dehumanize(k).to_sym => v)
    end
  end
...
Given /^(?:that|those) (.*) belongs? to that (.*)$/ do |child_or_children, parent|
  ...
  parent_instance.send("#{child_or_children.pluralize}=", [child_or_children_instance].flatten)
